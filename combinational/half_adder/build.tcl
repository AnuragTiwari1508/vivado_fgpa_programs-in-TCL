# ============================================================
# build.tcl - Independent Vivado build script for: half_adder
...
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for half_adder"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for half_adder, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for half_adder"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for half_adder"
    }

    puts "INFO: running implementation for half_adder"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for half_adder"
    }
    ...
    puts "DONE: half_adder synthesis+implementation+bitstream complete."
} else {
    puts "INFO: half_adder has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
