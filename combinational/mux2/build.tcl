# ============================================================
# build.tcl - Independent Vivado build script for: mux2
...
set_property top mux2_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for mux2"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for mux2, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for mux2"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for mux2"
    }

    puts "INFO: running implementation for mux2"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for mux2"
    }
    ...
    puts "DONE: mux2 synthesis+implementation+bitstream complete."
} else {
    puts "INFO: mux2 has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
