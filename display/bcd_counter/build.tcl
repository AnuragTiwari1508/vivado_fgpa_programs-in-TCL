# ============================================================
# build.tcl - Independent Vivado build script for: bcd_counter
# Category: display
...
set proj_name  "bcd_counter_proj"
...
set_property top bcd_counter_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for bcd_counter"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for bcd_counter, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for bcd_counter"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for bcd_counter"
    }

    puts "INFO: running implementation for bcd_counter"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for bcd_counter"
    }
    ...
    puts "DONE: bcd_counter synthesis+implementation+bitstream complete."
} else {
    puts "INFO: bcd_counter has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
