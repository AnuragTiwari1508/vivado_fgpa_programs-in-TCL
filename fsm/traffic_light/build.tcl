# ============================================================
# build.tcl - Independent Vivado build script for: traffic_light_demo_top
# Category: fsm
...
set proj_name  "traffic_light_demo_top_proj"
set proj_dir   [file join $origin_dir "vivado_proj"]
set part       "xc7s50csga324-1"
set top        "traffic_light_demo_top"
...
set_property top traffic_light_fsm_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for traffic_light_demo_top"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for traffic_light_demo_top, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for traffic_light_demo_top"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for traffic_light_demo_top"
    }

    puts "INFO: running implementation for traffic_light_demo_top"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for traffic_light_demo_top"
    }
    ...
    puts "DONE: traffic_light_demo_top synthesis+implementation+bitstream complete."
} else {
    puts "INFO: traffic_light_demo_top has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
