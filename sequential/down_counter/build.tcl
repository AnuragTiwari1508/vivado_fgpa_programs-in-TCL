# ============================================================
# build.tcl - Independent Vivado build script for: down_counter4
# Category: sequential
...
set proj_name  "down_counter4_proj"
set proj_dir   [file join $origin_dir "vivado_proj"]
set part       "xc7s50csga324-1"
set top        "down_counter4"
...
set_property top down_counter4_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for down_counter4"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for down_counter4, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for down_counter4"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for down_counter4"
    }

    puts "INFO: running implementation for down_counter4"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for down_counter4"
    }
    ...
    puts "DONE: down_counter4 synthesis+implementation+bitstream complete."
} else {
    puts "INFO: down_counter4 has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
