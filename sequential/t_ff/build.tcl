# ============================================================
# build.tcl - Independent Vivado build script for: t_ff
# Category: sequential
...
set_property top t_ff_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for t_ff"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for t_ff, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for t_ff"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for t_ff"
    }

    puts "INFO: running implementation for t_ff"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for t_ff"
    }
    ...
    puts "DONE: t_ff synthesis+implementation+bitstream complete."
} else {
    puts "INFO: t_ff has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
