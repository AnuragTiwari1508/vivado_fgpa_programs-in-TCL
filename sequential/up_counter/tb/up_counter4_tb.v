...
module up_counter4_tb;
    ...
    up_counter4 dut (.clk(clk), .rst(rst), .en(en), .q(q));
    always #5 clk = ~clk;
    ...
    initial begin
        ...
        if (q !== 0) begin $display("FAIL hold en=0: q=%0d",q); errors=errors+1; end else $display("PASS hold en=0");
        ...
        $display("PASS counted 1..15");

        @(posedge clk); #1; // rollover 15 -> 0
        if (q !== 0) begin $display("FAIL rollover: q=%0d",q); errors=errors+1; end
        else $display("PASS rollover 15->0");
