# ============================================================
# build.tcl - Independent Vivado build script for: button_debounce
# Category: utilities
...
set proj_name  "button_debounce_proj"
set proj_dir   [file join $origin_dir "vivado_proj"]
set part       "xc7s50csga324-1"
set top        "button_debounce"
...
set_property top button_debounce_tb [get_filesets sim_1]
update_compile_order -fileset sources_1

# ---- run behavioral simulation (if a TB exists) ----
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for button_debounce"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for button_debounce, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for button_debounce"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for button_debounce"
    }

    puts "INFO: running implementation for button_debounce"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for button_debounce"
    }
    ...
    puts "DONE: button_debounce synthesis+implementation+bitstream complete."
} else {
    puts "INFO: button_debounce has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
