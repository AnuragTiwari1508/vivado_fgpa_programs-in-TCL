# ============================================================
# build.tcl - Independent Vivado build script for: clock_divider
# Category: utilities
...
set proj_name  "clock_divider_proj"
set proj_dir   [file join $origin_dir "vivado_proj"]
set part       "xc7s50csga324-1"
set top        "clock_divider"
...
if {[llength $tb_files] > 0} {
    puts "INFO: launching behavioral simulation for clock_divider"
    catch {
        launch_simulation
        run all
        close_sim
    ...
    puts "INFO: no testbench found for clock_divider, skipping simulation."
}

# ---- synthesis / implementation / bitstream only for board-level (XDC-bearing) circuits ----
if {[llength $xdc_files] > 0} {
    puts "INFO: running synthesis for clock_divider"
    reset_run synth_1
    launch_runs synth_1 -jobs 4
    wait_on_run synth_1
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        error "ERROR: synthesis failed for clock_divider"
    }

    puts "INFO: running implementation for clock_divider"
    reset_run impl_1
    launch_runs impl_1 -to_step write_bitstream -jobs 4
    wait_on_run impl_1
    if {[get_property PROGRESS [get_runs impl_1]] != "100%"} {
        error "ERROR: implementation/bitstream failed for clock_divider"
    }
    ...
    puts "DONE: clock_divider synthesis+implementation+bitstream complete."
} else {
    puts "INFO: clock_divider has no XDC (pure logic-building-block circuit) - simulation only, no bitstream targeted."
}
